...
export function ProductsManagement() {
  ...
  const getAvailablePinOrders = (
    ...
    const occupied = new Set(
      products
        ...
        .map((p) => p.pinOrder)
        .filter(
          (order): order is 1 | 2 | 3 =>
            order === 1 || order === 2 || order === 3,
        ),
    );

    return ([1, 2, 3] as Array<1 | 2 | 3>).filter(
      (order) => !occupied.has(order),
    );
  };
    ...
    try {
      ...
      if (availableOrders.length === 0) {
        setStatusModal({
          ...
          title: "Pin Failed",
          message:
            "Maximum 3 pinned products allowed. Unpin one product first.",
        });
        return;
      }
      ...
      await productsApi.updatePin(product.id, {
        isPinned: true,
  ...
  const handlePinOrderChange = async (
    product: ProductDisplay,
    pinOrder: 1 | 2 | 3,
  ) => {
    try {
  ...
  const handleReorder = async (newOrder: ProductDisplay[]) => {
    try {
      ...
      // Fetch fresh data from backend to ensure alignment
      await fetchProducts();
    } catch (err: any) {
      ...
      setStatusModal({
        ...
        message:
          err.message ||
          "Failed to successfully reorder products. Please try again.",
      });
    }
  };
    ...
    },
    {
      ...
      render: (_value: boolean, row: ProductDisplay) => {
        ...
        const selectableOrders =
          row.isPinned && row.pinOrder
            ? (
                [...new Set([row.pinOrder, ...availableOrders])] as Array<
                  1 | 2 | 3
                >
              ).sort((a, b) => a - b)
            : availableOrders;
