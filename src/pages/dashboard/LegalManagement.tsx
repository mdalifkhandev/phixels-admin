...
export function LegalManagement() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.success) {
        setStatus({ type: "success", message: "Legal content updated successfully!" });
      } else {
        setStatus({ type: "error", message: response.message || "Failed to update content" });
      }
    } catch (error) {
  ...
  const addSection = () => {
    ...
    if (activeTab === "privacy") {
      setContent({ ...content, privacyPolicy: [...content.privacyPolicy, newSection] });
    } else {
      setContent({ ...content, termsConditions: [...content.termsConditions, newSection] });
    }
  };
  ...
  const activeSections = activeTab === "privacy" ? content.privacyPolicy : content.termsConditions;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage Privacy Policy and Terms & Conditions section by section.</p>
        </div>
      ...
      <div className="bg-[#0A0A0A] rounded-2xl border border-white/10 overflow-hidden">
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white flex items-center gap-2 text-capitalize">
            {activeTab === "privacy" ? "Privacy Policy" : "Terms & Conditions"} Sections
          </h2>
          <button
            onClick={addSection}
        ...
        <div className="p-6">
          {activeSections.length === 0 ? (
            <div className="text-center py-20 bg-white/5 rounded-xl border border-dashed border-white/10">
              ...
              <p className="text-gray-400 font-medium">No sections added yet.</p>
              <button 
                onClick={addSection}
